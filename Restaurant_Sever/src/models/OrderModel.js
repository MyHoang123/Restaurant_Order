...
const updateBillStatus = async (Id,result) => {
    await db.query('UPDATE billorder SET Status = ? WHERE IdTable= ? AND Status = 0',[1,Id],function(err,data){
        if(err || data.length <= 0) {
            result (null)
        }
        else {
...
const getProductOrder = function (IdType, callback)  {
    db.query(`SELECT p.Id,p.Name,'0' AS Price,ct.Name AS NameCate,p.Img,p.Sales,p.Visible,'visible' AS Status,ROUND(AVG(c.Star),1) AS Star FROM products p LEFT JOIN comment c ON p.Id = c.IdProduct INNER JOIN detailproducts d ON p.Id = d.IdProduct INNER JOIN categoris ct ON ct.Id = p.IdCategoris WHERE d.IdType = ? GROUP BY p.Id`,[IdType], function(err,data) {
       if(err || data.length <= 0) {
           callback (null)
       }
       else {
...
const getProductOrderAll = function (callback)  {
    db.query(`SELECT p.Id,p.Name,p.Price,ct.Name AS NameCate,p.Img,p.Sales,p.Visible,'visible' AS Status,ROUND(AVG(c.Star),1) AS Star FROM products p LEFT JOIN comment c ON p.Id = c.IdProduct INNER JOIN detailproducts d ON p.Id = d.IdProduct INNER JOIN categoris ct ON ct.Id = p.IdCategoris GROUP BY p.Id;`, function(err,data) {
       if(err || data.length <= 0) {
           callback (null)
       }
       else {
